...
#            print(np.size(Trinputs))
    ...
    for i in range(np.shape(Trinputs)[0]):
        ...
        Y_hat.append(scaledata(np.dot(np.transpose(Htrain), B),1,125))
...
def Score(YPred,Y):
    print('yyy',np.shape(Y))
    result=[];rul1=[];R=[];YPredLast=[];
    for i in range(np.shape(YPred)[0]):
        R.append(Y[i])
        YPredLast.append(YPred[i])
        result.append([result, YPredLast[i]])
        rul1.append([rul1, R[i]])
    print(result)
